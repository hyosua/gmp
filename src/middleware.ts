...
export default auth((req) => {
  ...
  if (isLoggedIn) {
    ...
    const isAccessingOtherSpace = Object.entries(rolePaths).some(
      ([r, path]) => {
    ...
    if (isAccessingOtherSpace) {
      console.log(
        `[AUTH] Unauthorized access attempt by ${role} to ${nextUrl.pathname}. Redirecting to ${targetPath}`,
      );
      return NextResponse.redirect(new URL(targetPath, nextUrl));
    }
  }
  ...
  function getRolePath(userRole: string | undefined): string {
    if (userRole === "ETUDIANT") return "/espace-etudiant/emploi-du-temps";
    if (userRole === "ENSEIGNANT") return "/espace-enseignant/emploi-du-temps";
